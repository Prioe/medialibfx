...
public class MediaInfoTest {
  ...
  private static void ByFileName(String FileName) throws Exception {
    ...
    */
    ...
    To_Display += MI.Get(MediaInfo.StreamKind.General, 0, 2,
    ...
    To_Display += MI.Get(MediaInfo.StreamKind.General, 0, "AudioCount",
        MediaInfo.InfoKind.Text, MediaInfo.InfoKind.Name);

    To_Display += "\r\n\r\nGet with Stream=Audio and Parameter=\"StreamCount\"\r\n";
    To_Display += MI.Get(MediaInfo.StreamKind.Audio, 0, "StreamCount",
        MediaInfo.InfoKind.Text, MediaInfo.InfoKind.Name);

    To_Display += "\r\n\r\nGet with Stream=General and Parameter=\"FileSize\"\r\n";
    To_Display += MI.Get(MediaInfo.StreamKind.General, 0, "FileSize",
        MediaInfo.InfoKind.Text, MediaInfo.InfoKind.Name);
  ...
  private static void ByBuffer_LocalFile(String FileName) throws Exception {
    ...
    do {
      ...
      if (MI.Open_Buffer_Continue_GoTo_Get() != -1) {
        ...
        MI.Open_Buffer_Init(From.length(), newPos); // Informing MediaInfo we
                                                    // have seek
      }
    } while (From_Buffer_Size > 0);

    // Finalizing
    MI.Open_Buffer_Finalize(); // This is the end of the stream, MediaInfo must
                               // finnish some work

    // Get() example
    String To_Display = new String();
    To_Display += "Get with Stream=General and Parameter=\"Format\": ";
    To_Display += MI.Get(MediaInfo.StreamKind.General, 0, "Format",
        MediaInfo.InfoKind.Text, MediaInfo.InfoKind.Name);
    To_Display += "\r\n";
    To_Display += "Get with Stream=Video and Parameter=\"Format_Settings_GOP\": ";
    To_Display += MI.Get(MediaInfo.StreamKind.Video, 0, "Format_Settings_GOP",
        MediaInfo.InfoKind.Text, MediaInfo.InfoKind.Name);
    System.out.println(To_Display);
    From.close();
  }
  ...
  private static void ByBuffer_URL(String FileName) throws Exception {
    ...
    // Parsing user input
    FileName = FileName.replace(" ", "%20");
    int FileName_Amazon_S3_End = FileName.indexOf("@s3.amazonaws.com/"); // Testing
                                                                         // if
                                                                         // Amazon
                                                                         // is
                                                                         // used
    ...
    if (FileName_Amazon_S3_End != -1) {
      ...
      int FileName_Amazon_S3_Middle = FileName.indexOf(":",
          FileName_Amazon_S3_Begin);
      String Amazon_S3_AWSAccessKeyId = FileName
          .substring(FileName_Amazon_S3_Begin, FileName_Amazon_S3_Middle);
      String Amazon_S3_AWSSecretAccessKey = FileName
          .substring(FileName_Amazon_S3_Middle + 1, FileName_Amazon_S3_End);
      FileName = FileName.replace(FileName.substring(FileName_Amazon_S3_Begin,
          FileName_Amazon_S3_End + 1), new String());
      Amazon_S3_Date = new SimpleDateFormat("EEE, d MMM yyyy HH:mm:ss Z",
          Locale.US).format(new Date());
      String Amazon_S3_ToSign = "GET\n\n\n" + Amazon_S3_Date + "\n"
          + new URL(FileName).getPath();
      Amazon_S3_Authorization = "AWS " + Amazon_S3_AWSAccessKeyId + ":"
          + calculateHmacSHA1(Amazon_S3_ToSign, Amazon_S3_AWSSecretAccessKey);
    }
    ...
    if (!Amazon_S3_Authorization.isEmpty()) {
      ...
      HttpFrom.setRequestProperty("Authorization", Amazon_S3_Authorization); // Amazon
                                                                             // only
    }
    HttpFrom.connect();
    ...
    long From_Length = Long
        .parseLong(HttpFrom.getHeaderField("Content-Length")); // .getContentLengthLong(),
                                                               // for 2GB+ file
                                                               // support is
                                                               // available only
                                                               // in Java 7 and
    ...
    do {
      ...
      if (MI.Open_Buffer_Continue_GoTo_Get() != -1) {
        ...
        MI.Open_Buffer_Init(From_Length, newPos); // Informing MediaInfo we have
                                                  // seek, TODO: take newPos
                                                  // from HTTP Content-Range
                                                  // value
      }
    } while (From_Buffer_Size > 0);
  ...
  private static String calculateHmacSHA1(String data, String secretKey)
    ...
    try {
      ...
      mac.init(new SecretKeySpec(secretKey.getBytes(), "HmacSHA1")); // Initialize
                                                                     // the HMAC
                                                                     // instance
                                                                     // with the
                                                                     // secret
      ...
      Result = DatatypeConverter
          .printBase64Binary(mac.doFinal(data.getBytes()));
    } catch (Exception e) {
